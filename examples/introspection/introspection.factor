...
: decode-gc-maps ( word -- seq )
    word>gc-info read-gc-maps ;

: gc-map-address ( word -- addr )
    word>gc-info scrub-bytes >c-ptr alien-address ;

: word-net-size ( word -- size )
    [ gc-map-address 16 align ] [ word-code drop ] bi - ;

: gc-map-size/word-size ( word -- ratio )
    [ word-code swap - ] [ word-net-size ] bi dupd - swap /  ;
